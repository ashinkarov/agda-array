...
--vec-mat-mul (imap a) (imap b) = imap λ iv → sum-1d (zipw _*_ (imap (λ jv → a (hd iv ∷ hd jv ∷ []) )) (imap b))
vec-mat-mul (imap a) b = imap λ iv → sum-1d (zipw _*_ (imap λ jv → a (iv ix++ jv)) b)

{-  where
    vec-f : _
    vec-f (i ∷ []) = reduce-1d (imap square) _+_ 0
      where
        square : _
        square (j ∷ []) = a (i ∷ j ∷ []) * b (j ∷ [])
-}
...
{-
iterate-1 : ∀ {a}{X : Set a}{n} → (Ix 1 (n ∷ []) → X) → (X → X → X) → X → X
iterate-1 {n = zero}  f _⊕_ neut = neut
iterate-1 {n = suc n} f _⊕_ neut = f (zero ∷ []) ⊕ iterate-1 (λ iv → f (suc (hd iv) ∷ [])) _⊕_ neut

red-iter : ∀ {a}{X : Set a}{n} → (f : Ix 1 (n ∷ []) → X) → (op : X → X → X) → (neut : X)
         → reduce-1d (imap f) op neut ≡ iterate-1 f op neut
